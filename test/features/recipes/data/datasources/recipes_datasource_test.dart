...
void main() {
  ...
  group('RecipesDataSourceImpl', () {
    ...
    test('getRecipesById returns RecipesModel on success', () async {
      when(mockDio.get('/lookup.php', queryParameters: {'i': id})).thenAnswer(
        (_) async => Response(
          data: recipesJson,
          statusCode: 200,
    ...
    test('getRecipesByName returns RecipesModel on success', () async {
      when(mockDio.get('/search.php', queryParameters: {'s': name})).thenAnswer(
        (_) async => Response(
          data: recipesJson,
          statusCode: 200,
      ...
      expect(result, isA<RecipesModel>());
      verify(
        mockDio.get('/search.php', queryParameters: {'s': name}),
      ).called(1);
    });

    test('getRecipesByCategory returns RecipesModel on success', () async {
      when(
        mockDio.get('/filter.php', queryParameters: {'c': category}),
      ).thenAnswer(
        (_) async => Response(
          data: recipesJson,
          statusCode: 200,
    ...
    test('getCategories returns CategoriesModel on success', () async {
      when(mockDio.get('/categories.php')).thenAnswer(
        (_) async => Response(
          data: categoriesJson,
          statusCode: 200,
